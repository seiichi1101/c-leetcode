...
public:
  int findCheapestPrice(int n, vector<vector<int>>& flights, int src, int dst, int k) {
    unordered_map<int, vector<vector<int>>> map_flights;
    for (auto&& f : flights) {
      if (map_flights.find(f[0]) != map_flights.end()) {
        map_flights[f[0]].push_back(f);
      } else {
        map_flights[f[0]] = {f};
      }
    }
    ...
    for (auto&& mf : map_flights[src]) {
      queue.push(pair{0, mf});
    }
    while (cnt <= k && !queue.empty()) {
      int size = queue.size();
      for (size_t i = 0; i < size; i++) {
        pair<int, vector<int>> front = queue.front();
        queue.pop();
        int cost = front.first + front.second[2];
        if (front.second[1] == dst)
          res = min(res, cost);
        if (cost < visited[front.second[1]]) {
          visited[front.second[1]] = cost;
          for (auto&& mf : map_flights[front.second[1]]) {
            queue.push(pair{cost, mf});
          }
        }
      }
...
int main() {
  Solution s;
  // vector<vector<int>> input1 = {{0, 1, 100}, {1, 2, 100}, {2, 0, 100}, {1, 3, 600}, {2, 3, 200}};
  // assert(s.findCheapestPrice(4, input1, 0, 3, 1) == 700);
  vector<vector<int>> input1 = {{1, 0, 5}, {2, 1, 5}, {3, 0, 2}, {1, 3, 2}, {4, 1, 1}, {2, 4, 1}};
  assert(s.findCheapestPrice(5, input1, 2, 0, 2) == 7);
  ...
  // assert(s.findCheapestPrice(17, input1, 13, 4, 13) == 700);
  return 0;
}
